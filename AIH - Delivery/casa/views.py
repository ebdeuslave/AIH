...
def livraison(request):
    ...
    # get checked objects using AJAX (see base.html)
    ...
    if request.method == 'POST':
        ...
        for id in pr_ids:
            ...
            pr.update(status=3, commentaire='PR/Inj/Inc')

    # Set Selected To Today
    if request.method == 'POST':
        today_ids = request.POST.getlist('today_ids[]')
        for id in today_ids:
            t = Livraison.objects.filter(pk=id)
            t.update(created=date.today())
...
def search(request):
    ...
    if request.GET:
        query = request.GET['q']
        if query != '' and query != ' ':
            results = Livraison.objects.filter(Q(zone__nom__icontains=query) | Q(phone__icontains=query) | Q(
                montant_DH__icontains=query) | Q(commentaire__icontains=query) | Q(n_commande__icontains=query)).order_by('-id', '-created')
            # calcul MT
            # All
            sum_mt_all = sum([mt.montant_DH for mt in results])
            # Livré
            sum_mt_delivered = sum(
                [mt.montant_DH for mt in results.filter(status=2)])
            # Retour
            sum_mt_returned = sum(
                [mt.montant_DH for mt in results.filter(status=3)])
            # Encours
            sum_mt_encours = sum(
                [mt.montant_DH for mt in results.filter(status=1)])
            # Statistics
            total = results.count()
...
class ViewPDF2(View):
    ...
    def get(self, request, *args, **kwargs):
        queryset = filtered.filter(status=1).order_by(
            'zone__nom', 'commentaire')
...
def add_livraison(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.info(
                request, f'Livraison : "{form.instance.site} - {form.instance.n_commande} - {form.instance.zone.nom} - {tele} - [{mt} Dh] {form.instance.commentaire}" a été ajouté')
...
def chartsView(request):
    ...
    try:
        ...
        total_cmds = [queryset.filter(
            created=f'{day[:4]}-{day[4:6]}-{day[6:]}').count() for day in date_list]
        total_mt = [sum([mt.montant_DH for mt in queryset.filter(
            created=f'{day[:4]}-{day[4:6]}-{day[6:]}')]) for day in date_list]
    except:
...
def monthlyChartsView(request):
    ...
    if request.GET:
        ...
        if query:
            ...
            try:
                months = [f'{YEAR}-01', f'{YEAR}-02', f'{YEAR}-03', f'{YEAR}-04', f'{YEAR}-05', f'{YEAR}-06',
                          f'{YEAR}-07', f'{YEAR}-08', f'{YEAR}-09', f'{YEAR}-10', f'{YEAR}-11', f'{YEAR}-12', ]
                monthly_total = [queryset.filter(
                    created__icontains=month).count() for month in months]
                monthly_mt = [sum([mt.montant_DH for mt in queryset.filter(
                    created__icontains=month)]) for month in months]
                sum_monthly_orders = sum(monthly_total)
                sum_monthly_mt = sum(monthly_mt)
                # retour
                monthly_total_retour = [queryset_retour.filter(
                    created__icontains=month).count() for month in months]
                monthly_mt_retour = [sum([mt.montant_DH for mt in queryset_retour.filter(
                    created__icontains=month)]) for month in months]
...
def zonesChart(request):
    ...
    queryset = filter.qs
    livraison_zones = [
        livraison.zone.nom for livraison in queryset if livraison.zone]
    counting_zones = {zone: livraison_zones.count(
        zone) for zone in livraison_zones}
    counting_zones = dict(sorted(counting_zones.items(),
...
def changeCMDState(request):
    ...
    livraisons = {livraison.n_commande: livraison.site.lower()
                  for livraison in queryset if livraison.n_commande}
    succesful = 0
    failed = 0
    failed_livraisons = []
    ...
    for cmd_id, website in livraisons.items():
        ...
        try:
            proc = subprocess.Popen(
                f"php changeStatus/updateCmdState.php {website} {cmd_id}", shell=True, stdout=subprocess.PIPE)
...
def livraisonChecker(request):
    ...
    NOT_SAME_PRICE = []
    NOT_CMD_ID = [
        livraison for livraison in queryset if not livraison.n_commande]
    REFUND = []
        ...
        try:
            ...
            for obj in queryset.filter(n_commande=cmd_id):
                if obj.montant_DH <= 0 and payment != 'cmi':
                    NOT_CMI.append([website, cmd_id, obj.phone, obj.montant_DH,
                                    orderMt, obj.zone.nom, obj.zone.livreur, obj.created])
                elif obj.montant_DH < 0 and payment == 'cmi':
                    REFUND.append([website, cmd_id, obj.phone, obj.montant_DH,
                                   orderMt, obj.zone.nom, obj.zone.livreur, obj.created])
                elif payment == 'Paiement comptant à la livraison (Cash on delivery)' and int(obj.montant_DH) != int(orderMt):
                    NOT_SAME_PRICE.append(
                        [website, cmd_id, obj.phone, obj.montant_DH, orderMt, obj.zone.nom, obj.zone.livreur, obj.created])
